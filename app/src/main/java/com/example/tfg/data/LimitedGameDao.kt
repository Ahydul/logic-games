...
interface LimitedGameDao {
    ...
    @Query("SELECT gameId, gameType, difficulty, startDate, numClues, timer, numErrors, seed from Game WHERE gameId = :id")
    ...
    @Query("SELECT gameId, gameType, difficulty, startDate, numClues, timer, numErrors, seed FROM Game WHERE endDate IS NULL and gameType = :type ORDER BY startDate DESC")
    fun getOnGoingGamesByType(type: Games): Flow<List<GameLowerInfo>>

    @Query("SELECT gameId, gameType, difficulty, startDate, numClues, timer, numErrors, seed FROM Game WHERE endDate IS NULL ORDER BY startDate DESC")
    fun getOnGoingGames(): Flow<List<GameLowerInfo>>

    @Query("SELECT gameId, gameType, difficulty, startDate, numClues, timer, numErrors, seed FROM Game WHERE endDate IS NOT NULL and gameType = :type ORDER BY endDate DESC")
